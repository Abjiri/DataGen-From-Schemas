...
let isEnumerated = content => "enumeration" in content
...
function parseList(st) {
   ...
   if (st.content.length == 1) {
      ...
      if (isGenType(st.content[0].built_in_base) && !isEnumerated(st.content[0].content)) return elem
      else {
      ...
      for (let i = 0; i < randomize(min,max); i++) {
         ...
         if (isGenType(st.content[type_ind].built_in_base) && !isEnumerated(st.content[type_ind].content)) {
...
function parseSimpleType(st) {
   ...
   return (isGenType(st.built_in_base) && !isEnumerated(content)) ? parsed : ("'" + parsed + "'")
}
