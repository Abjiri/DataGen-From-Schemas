...
function cleanSettings(settings, frontend) {
  ...
  if (!frontend) {
    if (!(typeof settings.extend_objectProperties == "string" && ["extend","overwrite"].includes(settings.extend_objectProperties))) return "O valor 'extend_objectProperties' das definições, relativo à extensão de propriedades repetidas nas chaves 'properties' e 'patternProperties', deve ser uma das seguintes strings: 'extend' ou 'overwrite'!\n\n'extend' - se as chaves tiverem propriedades repetidas, estende a schema de cada propriedade da chave-base com a respetiva schema da mesma propriedade da chave nova. Todas as propriedades originais da nova chave são atribuídas à chave-base.\n'overwrite' - se as chaves tiverem propriedades repetidas, substitui a schema de cada propriedade da chave-base pela respetiva schema da mesma propriedade da chave nova. Todas as propriedades originais da nova chave são atribuídas à chave-base."
    if (!(typeof settings.extend_schemaProperties == "string" && ["extend","overwrite"].includes(settings.extend_schemaProperties))) return "O valor 'extend_schemaProperties' das definições, relativo à extensão de chaves cujo valor é uma subschema ('propertyNames', 'additionalProperties', 'unevaluatedProperties', 'items' ou 'unevaluatedItems'), deve ser uma das seguintes strings: 'extend' ou 'overwrite'!\n\n'extend' - estende a schema da chave-base com a schema da chave nova.\n'overwrite' - substitui a schema da chave-base pela schema da chave nova."
    if (!(typeof settings.extend_prefixItems == "string" && ["extend","partial_overwrite","total_overwrite","append"].includes(settings.extend_prefixItems))) return "O valor 'extend_prefixItems' das definições, relativo à extensão da chave 'prefixItems', deve ser uma das seguintes strings: 'extend', 'partial_overwrite', 'total_overwrite' ou 'append'!\n\n'extend' - para todas as schemas que se encontram no mesmo índice, estende as da chave-base com as respetivas schemas da nova chave. Se a chave nova tiver mais elementos do que a base, os elementos extra são também concatenados.\n'partial_overwrite' - sobrescreve apenas as schemas da chave-base com uma schema correspondente no mesmo índice, na chave nova. Se a chave nova tiver mais elementos do que a base, os elementos extra são também concatenados.\n'total_overwrite - o valor da chave-base é apagado totalmente e substítuido pelo array do novo 'prefixItems'.\n'append' - os elementos do novo 'prefixItems' são concatenados aos da chave-base."
